...
// array in local storage for fake backend
const accountsKey = 'angular-signup-verification-boilerplate';
...
export class FakeBackendInterceptor implements HttpInterceptor {
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
            ...
            if (accounts.find(x => x.email === account.email)) {
                ...
                setTimeout(() => {
                    alertService.info(`
                        ...
                        <div><strong>NOTE:</strong> The fake backend displayed this "email", it would normally be sent to the user via email.</div>
                    `, { autoClose: false });
                }, 1000);
                return ok({});
            }
        ...
        function generateRefreshToken() {
            ...
            const tokenExpiry = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toUTCString();
            document.cookie = `fakeRefreshToken=${token}; expires=${tokenExpiry}; path=/`;
